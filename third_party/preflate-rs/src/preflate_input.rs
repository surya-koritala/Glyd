...
impl PlainText {
    ...
    /// `data` whose first `prefix_length` bytes are the dictionary that
    /// came before the text, the text starting at stream position
    /// `pos_offset`.
    pub fn with_prefix(data: Vec<u8>, prefix_length: usize, pos_offset: usize) -> Self {
        Self {
            data,
...
impl<'a> PreflateInput<'a> {
    ...
    /// An input standing at the start of the prefix (the dictionary),
    /// for feeding it to a predictor.
    pub fn from_prefix_start(v: &'a PlainText) -> Self {
