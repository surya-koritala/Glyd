...
impl TokenPredictor {
    ...
    /// Feeds `n` bytes at the input's position into the dictionary, as
    /// literals, and advances past them: a predictor started in the
    /// middle of a stream learns the window before it this way.
    pub fn seed(&mut self, input: &mut PreflateInput, n: u32) {
        self.state.rebase(input);
        for _ in 0..n {
