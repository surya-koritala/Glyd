...
impl BitReader {
    ...
    /// The position of the next unread bit in the stream, `consumed`
    /// being the bytes the reader has consumed so far.
    pub fn bit_position(&self, consumed: u64) -> u64 {
        (consumed + self.read_ahead as u64) * 8 - self.bits_left as u64
    }
