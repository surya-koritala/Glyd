...
impl DeflateParser {
    ...
    pub fn parse(&mut self, compressed_data: &[u8]) -> Result<DeflateContents> {
        ...
        match self.read_blocks_internal(&mut cursor, &mut blocks, &mut starts, &mut checkpoint) {
            Err(e) => {
                starts.truncate(blocks.len());
        ...
        loop {
            let (last, mode) = match self.state {
                DeflateParserState::StartBlock => {
                    starts.push(self.bit_reader.bit_position(reader.position()));
                    (
...
pub struct DeflateContents {
    ...
    /// where each block starts, in bits from the start of the data parsed
    pub block_bit_starts: Vec<u64>,
}
