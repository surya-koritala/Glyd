...
pub trait HashChainHolder {
    ...
    /// Starts the dictionary's positions at the input's: for a
    /// predictor that begins in the middle of a stream, before it adds
    /// anything.
    fn rebase(&mut self, input: &PreflateInput);
...
impl HashChainHolder for () {
    ...
    fn rebase(&mut self, _input: &PreflateInput) {}
