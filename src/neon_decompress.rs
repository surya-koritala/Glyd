...
pub unsafe fn decompress_neon(
    ...
    let v_base_lit = vdupq_n_u16(ESCAPE_BASE_LIT as u16);
    let v_base_match = vdupq_n_u16(esc_base_match as u16);
    ...
    'tokens: loop {
        'chunk: {
            ...
            let n_match = CHUNK
                - (vaddlvq_u8(vandq_u8(m_zero0, one)) + vaddlvq_u8(vandq_u8(m_zero1, one))) as usize;
            ...
            if vmaxvq_u8(cont) == 0 {
                e += fields;
                let ls0 = blend16(lit_esc0, vl0, v_base_lit, lit0, litl.as_mut_ptr());
                let ls1 = blend16(lit_esc1, vl1, v_base_lit, lit1, litl.as_mut_ptr().add(16));
                let ms0 = blend16(m_esc0, vm0, v_base_match, ml0, mll.as_mut_ptr());
                let ms1 = blend16(m_esc1, vm1, v_base_match, ml1, mll.as_mut_ptr().add(16));
