...
#include <string.h>
#include <assert.h>
#include "../include/glyd.h"

int main(void) {
    printf("====================================================\n");
    printf("  Testing Glyd C ABI Interface (Shared Library)\n");
    printf("====================================================\n");

    const char* version = glyd_version();
    printf("Glyd Version: %s\n", version);
    ...
    size_t max_comp_len = glyd_max_compressed_len(test_size);
    ...
    int64_t comp_bytes = glyd_compress(original, test_size, compressed, max_comp_len);
    ...
    int64_t decomp_bytes = glyd_decompress(compressed, (size_t)comp_bytes, restored, test_size);
    ...
    int64_t par_comp_bytes = glyd_compress_parallel(original, test_size, compressed, max_comp_len);
    ...
    memset(restored, 0, test_size);
    int64_t par_decomp_bytes = glyd_decompress_parallel(compressed, (size_t)par_comp_bytes, restored, test_size);
    ...
    memset(compressed, 0, max_comp_len);
    int64_t max_comp_bytes = glyd_compress_max(original, test_size, compressed, max_comp_len);
    ...
    memset(restored, 0, test_size);
    int64_t max_decomp_bytes = glyd_decompress(compressed, (size_t)max_comp_bytes, restored, test_size);
    ...
    memset(compressed, 0, max_comp_len);
    int64_t max_par_comp_bytes = glyd_compress_max_parallel(original, test_size, compressed, max_comp_len);
    printf("5b. Max-level parallel compress: written %lld bytes\n", (long long)max_par_comp_bytes);
    assert(max_par_comp_bytes > 0);

    memset(restored, 0, test_size);
    int64_t max_par_decomp_bytes = glyd_decompress_parallel(compressed, (size_t)max_par_comp_bytes, restored, test_size);
    ...
    int64_t err_comp = glyd_compress(original, test_size, compressed, 10);
    printf("7. Undersized buffer test: error code %lld\n", (long long)err_comp);
    assert(err_comp == -1);

    // Test 8: Null pointer safety
    int64_t err_null = glyd_compress(NULL, test_size, compressed, max_comp_len);
