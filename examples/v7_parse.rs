...
fn parse_all(d: &[u8], t: &mut DfastTables, seqs: &mut Vec<simd_stream_codec::v7_encode::Sequence>, lits: &mut Vec<u8>) -> usize {
    let mut n = 0;
    let mut off = 0;
    while off < d.len() {
        ...
        find_sequences_dfast(d, off, len, t, &mut reps, seqs, lits);
        n += seqs.len();
        off += len;
    }
    n
...
fn main() {
    ...
    if args.iter().any(|a| a == "--loop") {
        ...
        while start.elapsed().as_secs_f64() < secs {
            for (_, d) in &data {
                n += parse_all(d, &mut t, &mut seqs, &mut lits);
            }
        }
    ...
    for (f, d) in &data {
        let n_seq = parse_all(d, &mut t, &mut seqs, &mut lits);
        let pt = timed(runs, min_s, || {
            parse_all(d, &mut t, &mut seqs, &mut lits);
        });
