...
fn anatomy(name: &str, dict: &glyd::Dict, objects: &[&[u8]]) {
    let (mut raw, mut out, mut hdr, mut sub, mut secs, mut rawblocks, mut reuse_lit, mut reuse_seq, mut n, mut nseq, mut nlit) = (0usize, 0usize, 0usize, 0usize, [0usize; 5], 0usize, 0usize, 0usize, 0usize, 0usize, 0usize);
    for o in objects {
        let mut c = Vec::new();
        ...
        hdr += hl; n += 1; nseq += h.token_count as usize; nlit += h.literal_len as usize;
    ...
    println!("{name}: ratio {:.2}; per object {:.0} B = header {:.0} + subheader {:.0} + lit {:.0} + ll {:.0} + ml {:.0} + off {:.0} + extra {:.0}; raw blocks {rawblocks}/{n}, lit table reused {reuse_lit}, seq tables reused {reuse_seq}; {:.0} sequences and {:.0} literals per object",
        raw as f64 / out as f64, out as f64 / n as f64, hdr as f64 / n as f64, sub as f64 / n as f64, secs[0] as f64 / n as f64, secs[1] as f64 / n as f64, secs[2] as f64 / n as f64, secs[3] as f64 / n as f64, secs[4] as f64 / n as f64, nseq as f64 / n as f64, nlit as f64 / n as f64);
}
fn main() {
