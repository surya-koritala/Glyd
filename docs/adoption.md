...
stands on each (v0.14.3, September 2026). Ratio is not on the list
...
| Write speed at the default level, on all cores | The first number a platform team checks: `zstd -3 -T8` on their box. | **Not.** 0.7–0.8× zstd -3 -T8 on all cores, 0.45–0.8× on one. The double-fast loop is a third slower than zstd's on the same algorithm and the far-match pass is a third of the time on logs and events. This is the open engineering item. |
...
1. **Write speed**: the double-fast loop rewritten against the profile,
   then the far pass folded into it or made cheaper. This is the one
   objection every audience raises, and it is measurable.
