...
(cargo build --release --workspace && cargo build --release -p glyd-store --example s3cp) > ~/results/build.txt 2>&1 || { echo "BUILD FAILED" > ~/results/FAILED; touch ~/results/DONE; exit 1; }
